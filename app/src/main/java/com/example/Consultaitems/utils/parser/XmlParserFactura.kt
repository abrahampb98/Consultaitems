        ...
        fun parseAndUpdateDocumentCode(xmlData: String, database: SQLiteDatabase, pedido: String, context: Context):String  {
            ...
            try {
                while (parser.next() != XmlPullParser.END_DOCUMENT) {
                    if (parser.eventType == XmlPullParser.START_TAG && parser.name == "doc") {
                        parser.next()
            ...
            secuenciaValue?.let {
                ...
                val estado: String = "C"
                values.put("fa_coddocumento", it)
                val rowsAffected = database.update("fa_ws_cabproforma", values, "pr_codigo = ?", arrayOf(pedido))
                if (rowsAffected > 0) {
                    Log.d("SecuenciaXMLParser", "Updated successfully")
                } else {
                    Log.e("SecuenciaXMLParser", "Update failed")
                }
            } ?: Log.e("SecuenciaXMLParser", "No 'doc' tag found in the XML data")
