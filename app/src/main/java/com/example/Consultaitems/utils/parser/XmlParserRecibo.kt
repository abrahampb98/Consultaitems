    ...
    companion object {
        ...
        fun parseAndUpdateDocumentCode(xmlData: String, database: SQLiteDatabase, pedido: String, context: Context):String  {
            ...
            secuenciaValue?.let {
                ...
                val estado: String = "C"
                values.put("rc_codreciboA", it)
                values.put("rc_estado", estado)
                val rowsAffected = database.update("co_ws_reciboCobranzaConCab", values, "rc_codrecibo = ?", arrayOf(pedido))
                if (rowsAffected > 0) {
